...
if exist(strcat( OutP,'Betweenness.mat' )) == 2,
    load(strcat( OutP,'Betweenness.mat' ));
end
...
for cc = 1:length(Cond),
   for gg = 1:length(Groups),
      ...
      for ll = 1:length(Thr),
        ...
        %Create result mat
        Betweenness.(Cond{cc}).(Groups{gg}).(ext{ee}).(strcat('thr_',num2str(Thr(ll)*100))).mat = zeros(xx,numPart);
        ...
        for kk = 1:numPart,
            ...
            if strcmp('yes', MakeBinar),
                ...
                res = betweenness_wei(mat);
            end
             
            %Put measure in result mat
            Betweenness.(Cond{cc}).(Groups{gg}).(ext{ee}).(strcat('thr_',num2str(Thr(ll)*100))).mat(:,kk) = res;
        ...
        Betweenness.(Cond{cc}).(Groups{gg}).(ext{ee}).(strcat('thr_',num2str(Thr(ll)*100))).Avg = mean(Betweenness.(Cond{cc}).(Groups{gg}).(ext{ee}).(strcat('thr_',num2str(Thr(ll)*100))).mat,2);
        Betweenness.(Cond{cc}).(Groups{gg}).(ext{ee}).(strcat('thr_',num2str(Thr(ll)*100))).Std = std(Betweenness.(Cond{cc}).(Groups{gg}).(ext{ee}).(strcat('thr_',num2str(Thr(ll)*100))).mat,1,2);
...
save(strcat(OutP,'Betweenness.mat'),'Betweenness');