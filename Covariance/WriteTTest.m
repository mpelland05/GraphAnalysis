...
%Modified 14 march 2015, to include permutation
...
Results.tvalue.(ext) = zeros(length(fn),length(Seeds));
Results.pvalue.(ext) = zeros(length(fn),length(Seeds));
...
save(strcat(BaseP,otname,'_tvalue.mat'),'Results');

zer = zeros(1,length(Seeds));
Res = cat(1,Results.tvalue.(ext),zer,Results.pvalue.(ext));

xlswrite(strcat(BaseP,otname,'_Results.xls'),Res);
