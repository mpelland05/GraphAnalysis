...
for ii = 1:length(State),
   for jj = 1:length(Groups),
      ...
      for ll = 1:length(Thr),
        ...
        %Create result mat
        Efficiency.(State{ii}).(Groups{jj}).(strcat('thr_',num2str(Thr(ll)*100))).mat = zeros(xx,numPart);
        ...
        for kk = 1:numPart,
            ...
            Efficiency.(State{ii}).(Groups{jj}).(strcat('thr_',num2str(Thr(ll)*100))).mat(:,kk) = res;
        ...
        Efficiency.(State{ii}).(Groups{jj}).(strcat('thr_',num2str(Thr(ll)*100))).Avg = mean(Efficiency.(State{ii}).(Groups{jj}).(strcat('thr_',num2str(Thr(ll)*100))).mat,2);
        Efficiency.(State{ii}).(Groups{jj}).(strcat('thr_',num2str(Thr(ll)*100))).Std = std(Efficiency.(State{ii}).(Groups{jj}).(strcat('thr_',num2str(Thr(ll)*100))).mat,1,2);
        
      end
   end
end

save(strcat(BaseP,'Efficiency_partials.mat'),'Efficiency');